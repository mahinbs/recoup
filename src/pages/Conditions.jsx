...
const CategoryBadge = ({ category }) => {
    const getColors = (cat) => {
        switch (cat) {
            case "Neurological": return "bg-purple-50 text-purple-600 border-purple-100";
            case "Metabolic & Internal Medicine": return "bg-blue-50 text-blue-600 border-blue-100";
            case "Physical Rehab & Pain": return "bg-orange-50 text-orange-600 border-orange-100";
            case "Mental Health & Recovery": return "bg-green-50 text-green-600 border-green-100";
            case "Wellness & Longevity": return "bg-teal-50 text-teal-600 border-teal-100";
            case "Specialized Care": return "bg-slate-50 text-slate-600 border-slate-100";
            default: return "bg-gray-50 text-gray-600 border-gray-100";
        }
    };

    return (
        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getColors(category)}`}>
            {category}
        </span>
    );
};
            ...
            <Section className="pt-16 md:pt-24 pb-4 bg-white text-center border-b border-slate-100">
                <div className="max-w-4xl mx-auto px-4">
                    ...
                    <h1 className="text-4xl md:text-6xl font-extrabold text-primary-dark mb-6 tracking-tight">
                        Precision Care, <span className="text-primary italic font-serif">Personalized</span>
                    </h1>
                    <p className="text-slate-500 text-lg max-w-2xl mx-auto leading-relaxed">
                        Explore our 34+ specialized programs designed to target the root causes of chronic illness
                        and optimize your biological potential.
                    </p>
                </div>
                    ...
                    <AnimatePresence mode='wait'>
                        {filteredConditions.length === 0 ? (
                            ...
                            >
                                ...
                                <p className="text-slate-500">We couldn't find any protocol matching "{searchTerm}". Try adjusting your filters.</p>
                                <button 
                            ...
                            >
                                {Object.entries(groupedConditions).map(([cat, items], catIdx) => (
                                    <div key={cat} className="relative">
                                        <div className="flex items-center gap-4 mb-10">
                                            ...
                                            <span className="text-slate-400 font-medium text-sm">{items.length} Programs</span>
                                        </div>
