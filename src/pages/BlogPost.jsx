...
const BlogPost = () => {
  ...
  const post = useMemo(() => blogs.find((p) => p.id === id), [blogs, id]);

  const sanitizedBody = useMemo(() => {
    if (!post) return '';
    const raw = post.bodyHtml?.trim() ? post.bodyHtml : defaultArticleBodyHtml;
  ...
  return (
    <article className="bg-slate-50 min-h-screen pt-28 pb-20">
      <header className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-primary-dark leading-tight mb-8">{post.title}</h1>

        <p className="text-xl text-gray-600 leading-relaxed max-w-3xl mb-12">{post.excerpt}</p>

        <div className="w-full h-[400px] md:h-[500px] rounded-3xl overflow-hidden shadow-xl mb-12 bg-slate-200">
          {post.image ? (
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-12">
        <div className="lg:w-16 flex lg:flex-col gap-4 items-center pt-2">
          ...
          <button
            type="button"
            className="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:text-primary hover:border-primary hover:bg-primary/5 transition-all lg:mt-4"
          >
            <Share2 size={18} />
          </button>
        </div>
            ...
            .map((recentPost, idx) => (
              ...
              >
                <Link to={`/blog/${recentPost.id}`} className="group block">
                  <div className="aspect-[16/10] overflow-hidden rounded-2xl mb-4 relative bg-slate-200">
                    {recentPost.image ? (
                      <img
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-gray-400 font-bold uppercase tracking-wider mb-2">
                    ...
                    <span>•</span>
                    <span className="flex items-center gap-1">
                      <Clock size={12} /> {recentPost.readTime}
                    </span>
                  </div>
                  <h3 className="text-lg font-bold text-primary-dark group-hover:text-primary transition-colors leading-snug line-clamp-2">{recentPost.title}</h3>
                </Link>
              </motion.div>
            ))}
        </div>
